...
int codeGen(struct tnode *t){
	...
	switch(t->nt){
		...
		case nt_IFELSE:
			...
			freeReg();
			codeGen(t->down);
			fprintf(target_file,"JMP L%d --IEend\n",label2);
			fprintf(target_file,"L%d:\n",label1);
			codeGen(t->right);
			fprintf(target_file,"L%d:\n",label2);
			break;
		case nt_WHILE:
			...
			fprintf(target_file,"L%d:\n",label1);
			result=codeGen(t->left);
			fprintf(target_file,"JZ R%d,L%d --Wend\n",result,label2);
			freeReg();
			codeGen(t->right);
		...
		case nt_BREAK:
			if(top<0) {//puts("break not in loop");
				...
				fprintf(target_file, "JMP L%d --Bbreak\n",break2[top]);}
			break;
		case nt_CONTINUE:
			if(top<0) {//puts("continue not in loop");
						i=i;}
			else{//puts("continue");
				fprintf(target_file, "JMP L%d --Ccont\n",cont1[top]);}
			break;
		case nt_FUNC://-------------------------------------------------------------------
			//puts("function");
			regtemp=reg;
...
//*/
int fetchlocalloc(struct tnode *t){
	int i=getReg();
