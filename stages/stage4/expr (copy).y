...
Stmt			:READ '(' id ')' ';'								{$$=createtree(t_NULL,NULL,NULL,nt_READ,NULL,$3,NULL,NULL);}
				...
				|WRITE STRING ';'									{$$=createtree(t_NULL,NULL,NULL,nt_WRITE,NULL,$3,NULL,NULL);}
...
id				:ID							{$$=createtree(Lookup($1->str)->type,NULL,$1->str,nt_ID,NULL,NULL,NULL,Lookup($1->str));}
				|ID '[' NUM ']'				{$$=createtree(Lookup($1->str)->type,$3->num,$1->str,nt_ARR,NULL,NULL,NULL,Lookup($1->str));}
				|ID '[' NUM ']' '[' NUM ']'	{struct Gsymbol* t=Lookup($1->str);
											$$=createtree(Lookup($1->str)->type,(t->arr)*($3->num)+($6->num),$1->str,nt_ARR,NULL,NULL,NULL,Lookup($1->str));}
				|MUL ID						{$$=createtree($2->t,Lookup($2->str)->binding,$2->str,nt_SPTR,NULL,NULL,NULL,Lookup($2->str));}
				|MUL id						{int t=Lookup($2->str)->type==t_INPTR?t_INT:t_CHAR;
											if(t!=t_INPTR || t!=t_CHPTR)puts("Something's wrong!");
											$$=createtree(t,$2->num,$2->str,nt_SPTR,NULL,NULL,NULL,Lookup($2->str));}
				|'&' ID						{$$=createtree($2->t,Lookup($2->str)->binding,$2->str,nt_APTR,NULL,NULL,NULL,Lookup($2->str));}
				;
%%
