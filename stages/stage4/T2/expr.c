	...
	switch(t->nt){
		...
		case nt_ARR:
			...
			if(Lookup(t->str)==NULL){
				printf("Undeclared identifier:%s\n",t->str);
				exit(1);}
			if((Lookup(t->str)->size)<=t->num){
				printf("Accessing beyond declared!\n");
				exit(1);}
			printf("%d>=%d\n",Lookup(t->str)->size,t->num);
			struct Gsymbol* temp;
			temp=Lookup(t->str);
			printf("name:'%s'\ttype:'%d'\tsize:'%d'\tbinding:%d\n",temp->name,temp->type,temp->size,temp->binding);
			fprintf(target_file, "MOV R%d,[%d]\n",i,(Lookup(t->str)->binding)+t->num);
			return i;
			break;
		case nt_PLUS:
			puts("+");
		...
		case nt_ASGN:
			...
			if(Lookup(t->left->str)==NULL){
				printf("Undeclared identifier:%s\n",t->left->str);
				exit(1);}
			fprintf(target_file,"MOV [%d],R%d\n",Lookup(t->left->str)->binding,i );
			freeReg();
			break;
		case nt_LT:
			puts("<");
