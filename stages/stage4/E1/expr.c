...
void Install(char *name, int type, int size,int arr1, int arr2){
	...
	tail->arr1=arr1;
	tail->arr2=arr2;
	if(arr1==-1&&arr2==-1){tail->arr=0;}
	else if(arr1!=-1&&arr2==-1){tail->arr=1;;tail->type=t_ARR;}
	else if(arr1!=-1&&arr2!=-1){tail->arr=2;;tail->type=t_ARR;}
	else {printf("Please check Array declaration\n");exit(1);}
	binding=binding+size;
	printf("name:'%s'\ttype:'%d'\tsize:'%d'\tarr:%d\tarr1:%d\t:arr2:%d\n",tail->name,tail->type,tail->size,tail->arr,tail->arr1,tail->arr2);
...
int codeGen(struct tnode *t,FILE *target_file){
	...
	switch(t->nt){
		...
		case nt_ARR:
			...
			if(t->num==1){
				printf("1D\n");
				if(node->arr!=t->num){printf("Use arrays properly:(\n");exit(1);}
				if((node->size)<=t->arr[0]){
					printf("Accessing beyond declared:1D!\n");
					printf("%d<=%d\n",(node->size),t->num);
					exit(1);}
				fprintf(target_file, "MOV R%d,[%d]\n",i,(node->binding)+t->arr[0]);}
			if(t->num==2){
				printf("2D\n");
				if(node->arr!=t->num){printf("Use arrays properly:(\n");exit(1);}
				printf("%d:%d:%d:%d:%d\n",(t->arr[0]+1),(t->arr[1]+1),(t->arr[1]),(t->arr[0]),(t->arr[1]));
				if(t->arr[0]>=node->arr1 || t->arr[1]>=node->arr2){printf("Use arrays properly:(\n");exit(1);}
				int offset=(t->arr[0]+1)*(t->arr[1]+1)/(t->arr[1]+1)*(t->arr[0])+(t->arr[1]);
				if(offset>node->size){
					printf("Accessing beyond declared:2D!\n");
					printf("%d>%d\n",offset,node->size);
					exit(1);}
				fprintf(target_file, "MOV R%d,[%d]\n",i,(node->binding)+offset);}
			return i;
			break;
		case nt_PLUS:
			puts("+");
		...
		case nt_READ:
			...
			if(Lookup(t->down->str)==NULL){
				printf("Undeclared identifier:%s\n",t->down->str);
				exit(1);}
			Read(Lookup(t->down->str)->binding,target_file,ret);
			freeReg();
			break;
		case nt_WRITE:
			puts("write");
		...
		case nt_ASGN:
			...
			if(node->arr==0)
				...
				printf("1D\n");
				if(node->arr!=t->left->num){printf("Use arrays properly:(\n");exit(1);}
				if((node->size)<=t->arr[0]){
					printf("Accessing beyond declared:1D!\n");
					printf("%d<=%d\n",(node->size),t->num);
					exit(1);}
				fprintf(target_file, "MOV [%d],R%d\n",(node->binding)+t->arr[0],i);}
			else if(node->arr==2){
				printf("2D\n");
				if(node->arr!=t->left->num){printf("Use arrays properly:(\n");exit(1);}
				printf("%d:%d:%d:%d:%d\n",(t->arr[0]+1),(t->arr[1]+1),(t->arr[1]),(t->arr[0]),(t->arr[1]));
				if(t->left->arr[0]>=node->arr1 || t->left->arr[1]>=node->arr2){printf("Use arrays properly:(\n");exit(1);}
				int offset=(t->left->arr[0]+1)*(t->left->arr[1]+1)/(t->left->arr[1]+1)*(t->left->arr[0])+(t->left->arr[1]);
				printf("%d\n",offset);
				if(offset>node->size){
					printf("Accessing beyond declared:2D!\n");
					printf("%d>%d\n",offset,node->size);
					exit(1);}
				fprintf(target_file, "MOV [%d],R%d\n",(node->binding)+offset,i);}
			freeReg();
			break;
		case nt_LT:
			puts("<");
