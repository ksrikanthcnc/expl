struct tnode* createtree(int t, int num,char *str,int nt,struct tnode *l, struct tnode *d,struct tnode *r,struct Gsymbol *gentry,struct tnode *arglist){
		...
		case nt_FUNC:
			...
			while(ptemp!=NULL && atemp!=NULL){
				...
				if(ptemp->type!=atemp->t){
					printf("line:%d\ttype mismatch in Function call at '%d' parameter\n",line,q);
					printf("Declared:'%d'\nCalled:'%d'\n",ptemp->type,atemp->t);
					exit(1);}
				ptemp=ptemp->next;
				atemp=atemp->down;}
...
void GInstall(char *name, int type, int size){
	...
	if(GLookup(name)!=NULL){
		printf("line:%d\tvariable:'%s' redeclared\n",line,name);
		exit(1);
	}
	gtail->name=name;
	gtail->type=type;
...
void LocalParam(struct Lsymbol* lhead, struct Paramstruct* phead,struct Gsymbol *g){
	...
	if(pgtemp!=NULL || pltemp!=NULL){
		if(pgtemp!=NULL){
			printf("Extra Global Parameters Declared...\tline:%d\n",line);
			while(pgtemp==NULL){
				printf("name:'%s'\ttype:'%d'\n",pgtemp->name,pgtemp->type);
				pgtemp=pgtemp->next;
			}}
		else{
			printf("Extra Local Parameters Declared...\tline:%d\n",line);
			while(pltemp==NULL){
...
int getReg(){
	...
	if(reg>19){printf("Out of registers\n");
				exit(1);}
	return reg;}
void freeReg(){	printf("\t\tfreedReg:R%d\n",reg);reg--;}
