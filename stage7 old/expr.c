#define puts //puts
#define exit(a); raise(SIGINT); //exit(a);
/*
...
void GInstall(char *name, struct Typetable *type, int size){
	...
	if(GLookup(name)!=NULL){
		...
		exit(1);}
	new=malloc(sizeof(struct Gsymbol));
	new->name=name;
	new->type=type;
	new->size=size;
	if(size>0)
		new->binding=gbinding;
	else
		new->binding=0;
	...
	ghead=new;
	//printf("New entry is----\n");
	//printf("name:'%s'\ttype:'%d'\tsize:'%d'\tbinding:'%d'\tarr:%d\n",new->name,new->type,new->size,new->binding,new->arr);
	//printf("Install-ed...\n\n");
	;}
...
void PInstall(char* name, struct Typetable *type){
	...
	if(PLookup(name)!=NULL){
		...
		exit(1);}
	struct Paramstruct* new=malloc(sizeof(struct Paramstruct));
	new->name=name;
	new->type=type;
	new->next=phead;
	phead=new;
...
void LInstall(char *name, struct Typetable *type){
	...
	if(LLookup(name)!=NULL){
		...
		exit(1);}
	struct Lsymbol* new=malloc(sizeof(struct Lsymbol));
	new->name=name;
	new->type=type;
	new->binding=binding;
	new->next=lhead;
...
void TInstall(char *name,int size, struct Fieldlist *fields){
	...
	if(TLookup(name)!=NULL){
		...
		exit(1);}
	struct Typetable *new=malloc(sizeof(struct Typetable));
	new->name=name;
	new->size=8;
	new->fields=fields;
	new->next=thead;
...
struct Classtable* CInstall(char *name,struct Classtable *parent){
	...
	if(CLookup(name)!=NULL){
		printf("line:%d\tclass:'%s' redeclared\n",line,name);
		exit(1);}
...
void Class_Finstall(struct Classtable *cptr, char *type, char *name){
	...
	struct CFieldlist *new=malloc(sizeof(struct CFieldlist));
	new->name=name;
	new->Fieldindex=CFieldindex++;//-------------------init always
...
int codeGen(struct tnode *t){
	...
	switch(t->nt){
		...
		case nt_NUL:
			i=getReg();
			fprintf(target_file, "MOV R%d, 0 --null\n",i);
			return i;
		case nt_SELF:
