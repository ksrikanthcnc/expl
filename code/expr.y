%{
	...
	FILE *yyin;
	int reg;
	int flabel=1,mainflag=0,equiv=0,tuplesize=0,typesize=0,fieldindex;;
...
//-------------------------------------------------------------------------global
	GDeclBlock	:DECL GDeclList ENDDECL 	{generate();gprint();}
				|DECL ENDDECL 				{generate();puts("No Globals declared!");}
				;
	...
	GidList		:GidList ',' Gid
				|Gid;
	Gid			:ID										{GInstall($1->str,type,GetSize(type));}
...
//-------------------------------------------------------------------------body
	...
	Body		:BEG Slist RETURN Arth ';' END 	{//$$=$2;
												...
												puts("################################################################################################");}
				|BEG RETURN Arth ';' END 		{struct tnode *ret=createtree(GLookup(funcname)->type,NULL,NULL,nt_RET,NULL,$4,NULL,NULL,NULL,NULL);
												struct tnode *body=createtree(TLookup("void"),NULL,NULL,nt_NODE,$2,NULL,ret,NULL,NULL,NULL);
												funcGen(GLookup(funcname));
												printf("CodeGen-ing '%s'\n",funcname);
												codeGen(body);
	...
	Stmt		:READ '(' id ')' ';'								{$$=createtree(TLookup("void"),NULL,NULL,nt_READ,NULL,$3,NULL,NULL,NULL,NULL);}
				|WRITE '(' Arth ')' ';'								{$$=createtree(TLookup("void"),NULL,NULL,nt_WRITE,NULL,$3,NULL,NULL,NULL,NULL);}
				|WRITE STRING ';'									{$$=createtree(TLookup("void"),NULL,NULL,nt_WRITE,NULL,$2,NULL,NULL,NULL,NULL);}
				|id EQ Arth ';'										{$$=createtree(TLookup("void"),NULL,NULL,nt_ASGN,$1,NULL,$3,NULL,NULL,NULL);}
				...
				|INIT '(' ')' ';'									{$$=createtree(TLookup("void"),NULL,NULL,nt_INIT,NULL,NULL,NULL,NULL,NULL,NULL);}
				|id EQ ALLOC '(' ')' ';'							{$$=createtree(TLookup("void"),NULL,NULL,nt_ALLOC,NULL,$1,NULL,NULL,NULL,NULL);}
				|FREE '(' id ')' ';'								{$$=createtree(TLookup("void"),NULL,NULL,nt_FREE,NULL,$3,NULL,NULL,NULL,NULL);}
				;
	Arth		:Arth PLUS Arth			{$$=createtree(TLookup("int"),NULL,NULL,nt_PLUS,$1,NULL,$3,NULL,NULL,NULL);}
				|Arth MINUS Arth		{$$=createtree(TLookup("int"),NULL,NULL,nt_MINUS,$1,NULL,$3,NULL,NULL,NULL);}
				|Arth MUL Arth			{$$=createtree(TLookup("int"),NULL,NULL,nt_MUL,$1,NULL,$3,NULL,NULL,NULL);}
				|Arth DIV Arth			{$$=createtree(TLookup("int"),NULL,NULL,nt_DIV,$1,NULL,$3,NULL,NULL,NULL);}
				|Arth MOD Arth			{$$=createtree(TLookup("int"),NULL,NULL,nt_MOD,$1,NULL,$3,NULL,NULL,NULL);}
				|'(' Arth ')'			{$$=$2;}
				|NUM					{$$=$1;}
				|STRING					{$$=$1;}
				|id						{$$=$1;}
				|ID '(' ')'				{checkid($1);$$=createtree(GLookup($1->str)->type,NULL,$1->str,nt_FUNC,NULL,NULL,NULL,GLookup($1->str),NULL,NULL);q=$$;} 
				|ID '(' ArgList ')'		{checkid($1);
										$$=createtree(GLookup($1->str)->type,NULL,$1->str,nt_FUNC,NULL,NULL,NULL,GLookup($1->str),$3,NULL);};
	ArgList		:ArgList ',' Arth		{$3->down=$1;$$=$3;}
				|Arth					{$$=$1;};
	Bool		:Arth LT Arth			{$$ =createtree(TLookup("bool"),NULL,NULL,nt_LT,$1,NULL,$3,NULL,NULL,NULL);}
				|Arth LE Arth			{$$ =createtree(TLookup("bool"),NULL,NULL,nt_LE,$1,NULL,$3,NULL,NULL,NULL);}
				|Arth GT Arth			{$$ =createtree(TLookup("bool"),NULL,NULL,nt_GT,$1,NULL,$3,NULL,NULL,NULL);}
				|Arth GE Arth			{$$ =createtree(TLookup("bool"),NULL,NULL,nt_GE,$1,NULL,$3,NULL,NULL,NULL);}
				|Arth EE Arth			{$$ =createtree(TLookup("bool"),NULL,NULL,nt_EE,$1,NULL,$3,NULL,NULL,NULL);}
				|Arth NE Arth			{$$ =createtree(TLookup("bool"),NULL,NULL,nt_NE,$1,NULL,$3,NULL,NULL,NULL);};
//-------------------------------------------------------------------------id
	//struct tnode* createtree(struct Typetable *type, int num,char *str,int nt, struct tnode *l, struct tnode *r,struct tnode *d,struct Gsymbol *gentry,struct tnode *arglist,struct Lsymbol *lentry);
	id			:ID							{checkid($1);
												$$=createtree(TLookup($1->str),NULL,$1->str,nt_ID,NULL,NULL,NULL,GLookup($1->str),NULL,LLookup($1->str));}
				|ID '[' Arth ']'			{checkid($1);
											$$=createtree(TLookup($1->str),$3->num,$1->str,nt_ID,NULL,$3,NULL,GLookup($1->str),NULL,LLookup($1->str));}
/*				|ID '[' NUM ']' '[' NUM ']'	{checkid($1);
	...
	Field		:Field '.' ID 	{	$$->t=$3->t;} 
				|ID '.' ID		{	checkid($1);
									...
									$$=createtree(ftemp->type,ftemp->fieldIndex,$3->str,nt_ID,NULL,NULL,NULL,GLookup($1->str),NULL,LLookup($1->str));}
				;
%%
...
void gprint(){
	...
	while(gtemp!=NULL){
		if(gtemp->size>0){
			printf("%d.name:'%s'\ttype:'%s'\tsize:'%d'\tarr:%d\n",i++,gtemp->name,gtemp->type->name,gtemp->size,gtemp->arr);
		if(gtemp->paramlist){
