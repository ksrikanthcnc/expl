...
//-------------------------------------------------------------------------class
	...
	*/
	...
	Flist		:Type Cid ';' 
				;
	//CType		:ID	{ctemp=CLookup($1->str);if(ctemp==NULL) {puts("error");exit(1);}}
	Cid			:Cid ',' ID			{Class_Finstall(Cptr,typename,$3->str);} //Installing the field to the class
				|ID					{Class_Finstall(Cptr,typename,$1->str);} //Installing the field to the class
				;
	MethodDecl 	:MethodDecl MDecl
				|MDecl//----------needed
				;
